<?php
    ...
    */
    ...
    function inventario() {
        ...
        $pdf->Cell(76, 3, '', 0, 1, 'C');
        $pdf->Cell(275, 7, 'INVENTARIO FARMACIA MULTIMEDIAL', 0, 1, 'C');
        $pdf->Setfont('Arial','B',10);
        $pdf->Cell(275, 4, 'Fecha: ' . date('d/m/y'), 0, 1,'C');
        $pdf->Cell(76, 5, '', 0, 1, 'C');

        $pdf->Setfont('Arial','B', 11);
        $pdf->Cell(40, 7, 'Codigo ', 1, 0,'L');
        $pdf->Cell(160, 7, 'Descripcion ', 1, 0,'L');
        $pdf->Cell(32, 7, 'Existencias ', 1, 0,'L');
        $pdf->Cell(45, 7, 'Precio unitario(Bs.)', 1, 1,'L');

        $pdf->Setfont('Arial','', 10);
        
        foreach ($medicines as $i => $value) {
            $pdf->Cell(40, 7, get_medicine_id($value), 1, 0,'L');
            $pdf->Cell(160, 7, $value, 1, 0,'L');
            $pdf->Cell(32, 7, get_remaining_medicine_quantity($value), 1, 0,'L');
            $pdf->Cell(45, 7, get_medicine_price($value), 1, 1,'L');
        }
